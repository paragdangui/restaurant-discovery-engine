		...
		>
			<div class="flex min-h-screen items-center justify-center p-4">
				...
				>
					<!-- Header -->
					<div
						class="sticky top-0 bg-white border-b border-surface-200 px-6 py-4 z-10"
					>
								...
								<div class="flex items-center mt-1 space-x-4">
									<div v-if="restaurant?.rating" class="flex items-center">
										<div class="flex items-center">
											...
											<span class="ml-1 text-sm font-medium text-surface-900">{{
												restaurant.rating
											}}</span>
										</div>
										<span
											v-if="restaurant.reviewCount"
											class="ml-1 text-xs text-surface-500"
										>
								...
								<button
									...
									:class="[
										...
										isFavorite
											...
											: 'bg-surface-100 text-surface-600 hover:text-red-500',
									]"
								>
									<HeartIcon
						...
						<div class="p-6 space-y-6">
							...
							<div class="flex items-center justify-between">
								<div class="flex items-center space-x-3">
									...
									<span
										v-if="distance"
										class="px-3 py-1 bg-surface-100 text-surface-800 text-sm font-medium rounded-full"
									>
										{{ distance }}
									</span>
								</div>
							...
							<div>
								...
								<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<!-- Hours -->
									<div
										v-if="restaurant?.hours && restaurant.hours.length > 0"
										class="flex items-start space-x-3"
									>
										<ClockIcon
											class="w-5 h-5 text-surface-400 mt-0.5 flex-shrink-0"
										...
										<div>
											...
											<div class="text-sm text-surface-600">
												...
												<div v-if="showAllHours" class="mt-2 space-y-1">
													<div
														v-for="(hours, index) in restaurant.hours"
														:key="index"
													>
														<span class="font-medium"
															>{{ getDayName(hours.day) }}:</span
														>
														{{ formatHours(hours) }}
													</div>
												</div>
							...
							<div
								v-if="aiInsights"
								class="bg-gradient-to-r from-secondary-50 to-info-50 rounded-lg p-4"
							>
								...
								<div v-if="aiInsights.reviewSummary" class="mb-4">
									<h4 class="text-sm font-medium text-surface-900 mb-2">
										What customers are saying
									</h4>
									<p class="text-sm text-surface-600">
										{{ aiInsights.reviewSummary }}
									</p>
								</div>
